...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    image_hdr = cam_info.image_hdr
    exp_time = cam_info.exposure
    ...
    if not args.dataloader:
        ...
        if image_hdr is not None:
            image_hdr = torch.from_numpy(image_hdr).permute(2, 0, 1)  # [C, H, W]
            image_hdr = F.interpolate(image_hdr[None], size=(resolution[1], resolution[0]), mode='bicubic', align_corners=False)[0]
            image_hdr = torch.clamp(image_hdr / image_hdr.max(), 0.0, 1.0)
            assert not torch.isnan(image_hdr).any(), f"gt_hdr contains nan values"
            # image_hdr = cv2.resize(image_hdr, resolution, interpolation=cv2.INTER_LINEAR)
            # image_hdr = image_hdr.transpose(2, 0, 1)
    else:
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  meta_only=args.dataloader, image_hdr=image_hdr, exp_time=exp_time
                  )
